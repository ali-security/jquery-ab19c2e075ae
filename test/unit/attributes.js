...
test( "jQuery.find.attr with a non-lowercase boolean attribute should not recurse", function() {
	expect( 3 );
	...
	jQuery.each({
		...
		try {
			strictEqual( jQuery.find.attr( elem, original ), lowercased,
				"jQuery.find.attr( elem, '" + original + "' ) returns the lowercased name" );
		} catch ( e ) {
...
test( "non-lowercase boolean attribute selectors should not crash", function() {
	expect( 3 );
	...
	jQuery.each([ "[Checked]", "[requiRed]", "[AUTOFOCUS]" ], function( i, selector ) {
		try {
			equal( elems.filter( selector ).length, 2,
				"Both elements match the '" + selector + "' selector" );
		} catch ( e ) {
			ok( false, "Filtering by the '" + selector + "' selector threw" );
		}
	});
});
